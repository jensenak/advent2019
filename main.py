...
testData = [
    {"A": "R8,U5,L5,D3", "B": "U7,R6,D4,L4", "D": 6, "X": 20, "Y": 20},
    {
...
class wiremap:
    def __init__(self, x, y):
        ...
        self.gridy = y
        self.xpoints = []
        self.wmap = [[0 for y in range(y * 2)] for x in range(x * 2)]
        self.setpoint = lambda x, y: 1

    def crosspoint(self, x, y):
        if self.wmap[x][y] == 1:
            self.xpoints.append(abs(self.gridx - x) + abs(self.gridy - y))
            return 2
        return 0
...
for t in testData:
    ...
    if w.result() == t["D"]:
        ...
        print(f"Failed! Expected {t['D']} but got {w.result()}")
        w.show()

w = wiremap(12000, 10000)
